...
app.post('/api/tree-shake', async (req, res) => {
    ...
    try {
        ...
        while (isLoopActive && loopSafety < 200) {
            ...
            if (rand < 0.10) {
                crashPoint = 1.00; // ১০% রাউন্ডে ইনস্ট্যান্ট ১.০০ ক্র্যাশ লস ট্র্যাপ!
            } else {
                crashPoint = parseFloat((1.01 + (0.05 / (Math.random() + 0.01))).toFixed(2));
            }

            if (crashPoint > 100.00) crashPoint = 100.00; // সর্বোচ্চ সীমা ১০০ গুণ লক!
        ...
        // বাজি ধরার সাথে সাথে ওয়ালেট থেকে বাজি এমাউন্ট ইনস্ট্যান্ট কেটে নেওয়া বর্ম
        let phpPayload = {
            action: "bet",
            username: userId,
            amount: reqAmount,
...
io.on('connection', (socket) => { 
    console.log("Player connected to Royal Money Tree Crash Engine!"); 
});

// টাকার গাছ ক্র্যাশ গেম নিজস্ব কাস্টম ৬০০০ পোর্টে গ্র্যান্ড ফিনালে নিয়নে অন ফায়ার ভাই ভাই!
const PORT = process.env.PORT || 6000; 
server.listen(PORT, () => { 
    console.log(`🎡 Royal Money Tree Crash Engine Running on port ${PORT}`); 
});
